...
def plot(ax, methods, colors, opt_file, base_path, y_lim, label, title):
    ...
    plt.xlim([2**0, 2**7])
    plt.ylim(y_lim)
    plt.plot(xaxis, 2 ** 4 / np.sqrt(xaxis), 'k', label='$1/\sqrt{t}$', linewidth = 2)
    plt.plot(xaxis, 2 ** 4 / xaxis, 'k--', label='$1/t$', linewidth = 2)
    plt.plot(xaxis, 2 ** 4 / xaxis**2, 'k-.', label='$1/t^2$', linewidth = 2)

    ax.text(0.0, 1.02, label, transform=ax.transAxes, fontsize=fontsize)
...
plot(ax, methods, COLORS, name, "stochastic_grid/results/",(2**(-14),2**6), 'a)', 'deterministic')
...
methods = ["adaptive-1-prob-"+str(p),"adaptive-prob-"+str(p),"tracking-prob-"+str(p), "density-prob-"+str(p), "random-prob-"+str(p)]
name = "stochastic_grid/results/opt+"+str(np.round(float(p),2))+".txt"
plot(ax, methods, COLORS, name, "stochastic_grid/results/",(2**(-14),2**6), 'b)', 'stochastic (p=0.1)')
...
plt.savefig("figs/grids-subplots.png",dpi = 100, bbox_inches = 'tight',pad_inches = 0)
plt.show()
